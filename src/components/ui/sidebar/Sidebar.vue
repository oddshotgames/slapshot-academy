...
<template>
	<div v-if="collapsible === 'none'"
		:class="cn('flex h-full w-(--sidebar-width) flex-col bg-sidebar text-sidebar-foreground', props.class)" :style="{
	...
	<div v-else class="group peer hidden md:block" :data-state="state"
		:data-collapsible="state === 'collapsed' ? collapsible : ''" :data-variant="variant" :data-side="side" :style="{
			'--sidebar-width': SIDEBAR_WIDTH,
			'--sidebar-width-icon': SIDEBAR_WIDTH_ICON,
		}">
		...
		<div :class="cn(
			'duration-200 fixed top-(--header-height) bottom-0 z-10 hidden h-[calc(100vh-var(--header-height))] w-(--sidebar-width) transition-[left,right,width] ease-linear md:flex',
			side === 'left'
